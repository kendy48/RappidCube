...
class CubeController extends BaseController {
    ...
    public function solveCube()
    {
        try {
            ...
            for ($x = 1; $x <= $qtyTest; $x++) {
                ...
                $cube = $this->initializeCube($cubeSize);
                for ($i = $baseIndex + 1; $i <= $baseIndex + $qtyQuery; $i++) {
                    $result = $result . $this->solveQuery($cube, $inputParams[$i]) . "\n";
                }
    ...
    private function initializeCube($size){
    ...
    private function solveQuery(&$cube, $query){
        if (strpos(strtoupper($query), "UPDATE") !== false) {
            return $this->update($cube, $query);
        } else if (strpos(strtoupper($query), "QUERY") !== false) {
            return $this->query($cube, $query);
        }
    }

    private function update(&$cube, $query){
    ...
    private function query($cube, $query){
        $params = explode(" ",$query);
        $x1 = $params[1]-1;
        $y1 = $params[2]-1;
        $z1 = $params[3]-1;
